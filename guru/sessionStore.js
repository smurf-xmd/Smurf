...
async function init(config) {
    ...
    if (dbType === 'mongodb') {
        try {
            ...
            mongoModel = mongoose.models.GuruSession || mongoose.model('GuruSession', sessionSchema);
...
async function saveSession(fullSessionString) {
    ...
    if (storageBackend === 'mongodb') {
        ...
        await pgPool.query(
            'INSERT INTO guru_sessions (short_id, data) VALUES ($1, $2)',
...
async function getSession(id) {
    ...
    if (storageBackend === 'mongodb') {
        ...
        const result = await pgPool.query(
            'SELECT data FROM guru_sessions WHERE short_id = $1',
            [safeId]
        );
